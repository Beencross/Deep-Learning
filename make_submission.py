...
class Dataset:    ### Creation of the Dataset class for each 1108 genetic disturbances ####
    CLASSES = 1108

    def __init__(self, path):          #### gestion 
        self.data = {}
        self.controls = {}
        ...
        for is_control, file in [(0, 'train.csv'), (1, 'train_controls.csv'), (1, 'test_controls.csv')]:
            ...
            for row in csv.iterrows():   ### iterrows generate both the index and the line ###
                r = row[1]
                (self.controls if is_control else self.data)[self.split(r.id_code)] = r.sirna  ### Separate controls ID and data ID ###
    ...
    @staticmethod
    def split(id_code): ### Return (cell_type, experiment number of given cell type, plate number) ###
    ...
    def _get_groups(self): ### Calculate class groups that are on plates and assignment for the labeled set ###
    ...
    def assign_groups(self, data): ### Find group assignments as dictionary in format {code_id: list_of_classes} ###
    ...
    def accuracy(self, data):
        if isinstance(data, dict):     ## Check if the data is an dictionary ###
            data = data.items()
...
class PredictionGroup:
    ...
    def normalize(self):     ### function for centering the data and reducing it with the standard deviation ###
